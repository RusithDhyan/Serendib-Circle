...
export default function BlogContent({ blogId }) {
  ...
  async function generateChecksum(timestamp) {
    const API_KEY = process.env.NEXT_PUBLIC_KEY; // public key!
    const raw = timestamp + API_KEY;
  ...
  const fetchContent = useCallback(async () => {
    ...
    const res = await fetch(`/api/blog-content?t=${t}&cs=${cs}&blogId=${blogId}`);
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`/api/blog-content/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="mt-4 space-y-4 shadow-md py-10 px-4">
      ...
      {!currentUser?.permissions?.canReadBlogContent ? (
      ...
      {blogContent?.map((content) => (
        <div key={content._id} className=" pb-4">
          ...
          <div className="space-x-2 mt-2 flex items-center justify-end gap-2">
            {currentUser?.permissions?.canUpdateBlogContent && (
            <button
              onClick={() => {
            ...
            {currentUser?.permissions?.canDeleteBlogContent && (
            <button
