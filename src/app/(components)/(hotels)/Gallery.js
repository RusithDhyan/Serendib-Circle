...
export default function Experience({ hotelId }) {
  ...
  return (
    <div className="mt-4 space-y-4 shadow-md py-10 px-4">
      {galleries.length === 0 && (
      <p className="text-gray-500 text-center">No galleries available for this hotel.</p>
    )}

    {!currentUser?.permissions?.canReadGallery ? (
      ...
      {galleries.map((gallery) => (
        ...
        <div className="space-x-2 flex items-center justify-end gap-2 mt-2">
          {currentUser?.permissions?.canUpdateGallery && (
            <button
              onClick={() => {
          ...
          {currentUser?.permissions?.canDeleteGallery && (
            <button
