...
export default function AddUsers({ onClose, editingAddUsers }) {
  ...
  if (!currentUser || !currentUser.role) return roles;

  const currentLevel = ROLE_LEVEL[currentUser.role.toLowerCase()];
  const editableRole = editingAddUsers?.role?.toLowerCase() || "";

  return roles.filter(
    (r) =>
      ROLE_LEVEL[r.value] < currentLevel || r.value === editableRole
  );
}, [currentUser, editingAddUsers]);
  ...
  useEffect(() => {
    if (editingAddUsers) {
      setForm({
        fullname: editingAddUsers.fullname || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingAddUsersId) {
        res = await fetch(`/api/users/${editingAddUsersId}`, {
          method: "PUT",
          body: formData,
        });
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <small className="text-red-500">You don't have permission to add higher roles</small>
            </div>
