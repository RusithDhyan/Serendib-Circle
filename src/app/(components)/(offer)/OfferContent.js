...
import AddOfferContent from "./(form)/AddOfferContent";
import { CheckCircle } from "lucide-react";

export default function OfferContent({ offerId }) {
  const [showPopup, setShowPopup] = useState(false);
  const [editingOfferContent, setEditingContent] = useState(null);
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`/api/offer-content/${id}`, { method: "DELETE" });
    ...
    <div className="mt-4 space-y-4 shadow-md py-10 px-4">
      {contents.length === 0 && (
        <p className="text-gray-500 text-center">
          No offer content available for this site.
        </p>
      )}
