...
const origin = req.headers.get("origin");
  ...
  const bg_image = formData.get("bg_image");
    ...
    const saveImage = async (file) => {
      ...
      const imageUrl = await saveImage(content_image);
      const bgImageUrl = await saveImage(bg_image);
 
  try {
    await connectDB();
    const newAboutContent = await AboutTop.create({
      title,
      vision_title,
      description,
      body_title,
...
export async function GET(req) {
  ...
  try {
    await connectDB();
    const aboutContent = await AboutTop.findOne({});
