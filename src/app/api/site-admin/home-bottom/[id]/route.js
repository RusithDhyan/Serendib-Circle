...
export async function PUT(request, context) {
  ...
  try {
    ...
    const updatedHomeBottomContent = await HomeBottom.findByIdAndUpdate(
      id,
      updates,
      {
        new: true,
    ...
    if (!updatedHomeBottomContent) {
      return NextResponse.json(
        { message: "HomeBottomContent not found" },
        { status: 404 }
      );
    }

    return NextResponse.json(
      { success: true, homeContent: updatedHomeBottomContent },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
...
export async function DELETE(request, context) {
  ...
  try {
    await connectDB();
    const deletedHomeBottomContent = await HomeBottom.findByIdAndDelete(
      id
    ).lean();
    ...
    return NextResponse.json(
      { message: "HomeBottomContent deleted" },
      { status: 200 }
    );
  } catch (error) {
    return NextResponse.json(
      { message: "Failed to delete HomeBottomContent", error },
      { status: 500 }
    );
  }
}
