...
export async function GET(req) {
  ...
  try {
    await connectDB();
    const homeBottom = await HomeBottom.findOne({});
    const response = NextResponse.json({ success: true, data: homeBottom });
...
export async function POST(req) {
  ...
  try {
    ...
    const imageUrl = await saveImage(bg_image);

    const home_content = await HomeBottom.create({
      title,
      description,
      bg_image: imageUrl,
    });
