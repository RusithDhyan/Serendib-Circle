...
export async function POST(req) {
  ...
  // --- 4. Process the rest of the form ---
  ...
  try {
    ...
    const newContactContent = await ContactContent.create({
      title,
      description,
      email,
      phone,
...
export async function GET(req) {
  ...
  try {
    await connectDB();
    const contactContent = await ContactContent.findOne({});
    let res = NextResponse.json({ success: true, data: contactContent });
    return setCorsHeaders(res, origin);
  } catch (error) {
