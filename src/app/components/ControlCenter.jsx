...
export default function ControlCenter({ user, onUpdate }) {
  ...
  const [roomPreferences, setRoomPreferences] = useState(user.roomPreferences || []);
  const [explorePreferences, setExplorePreferences] = useState(user.explorePreferences || []);
  const [isSaving, setIsSaving] = useState(false);
  const dietaryOptions = ['Vegetarian', 'Vegan', 'Gluten-Free', 'Dairy-Free', 'Halal', 'Kosher', 'No Seafood', 'Nut Allergies'];
  const roomOptions = ['Ocean View', 'City View', 'High Floor', 'Low Floor', 'King Bed', 'Twin Beds', 'Smoking', 'Non-Smoking', 'Quiet Location', 'Near Elevator'];
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/user', {
        ...
        body: JSON.stringify({ dietaryPreferences, roomPreferences, explorePreferences }),
      });
      if (response.ok) {
  ...
  return (
    <div className="card">
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-lg">
        <div className="text-sm font-semibold text-gray-900 mb-2">Stay History</div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-xs text-gray-600">Total Stays</div>
            <div className="text-2xl font-bold text-serendib-primary">{user.totalStays}</div>
          </div>
          <div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {exploreOptions.map((option) => (
              <button key={option} onClick={() => togglePreference(explorePreferences, setExplorePreferences, option)}
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${explorePreferences.includes(option) ? 'bg-serendib-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                {option}
              </button>
            ))}
          </div>
